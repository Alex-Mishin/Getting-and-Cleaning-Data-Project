...
##from a folder named "UCI HAR Dataset", avaiable online at:
...
y_train <- read.table("UCI HAR Dataset\\train\\y_train.txt", 
                      col.names="activity_id", colClasses= "factor")
subject_train <- read.table("UCI HAR Dataset\\train\\subject_train.txt", 
                            col.names="subject_id", colClasses= "factor")
...
grouped_average <- mean_std %>% 
                        group_by(activity_name, subject_id) %>% 
                              summarise_each(funs(mean))

write.table(grouped_average, file="grouped_average.txt", row.name=FALSE)
